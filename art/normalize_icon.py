...
注意： 着色铁律（2026-09 实战教训，务必先读）：
  本脚本的几何步骤（裁 alpha bbox / 等比缩放 / 居中）**逐通道不改 RGB**，是安全的；
  但 `color` 参数一旦赋值就是**把 R=G=B 整个覆盖掉**（`unit_icon` 的 color=255 就是
  故意做白色剪影）。**彩色类别（建筑/区域/项目/资源/伟人/领袖…）必须 color=None**，
  且**严禁再加任何 levels / gamma / 亮度 / 对比度 / 饱和度 / 色相调整**——那会改色相，
  是返工项（本项目建筑图标曾因加了一步 levels+gamma 被判「色相变了」而全部重做）。
  需要"提亮暗图"时，先问用户，不要自作主张。
