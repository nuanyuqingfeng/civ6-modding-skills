...
用途：工坊封面底图与徽记、一般风格插图。**不适合图标** —— 图标剪影走
`art/normalize_icon.py`（把已有主体图裁边/等比/居中后涂成白色剪影：`--color 255`，
unit 图标用 `--role unit_icon`）；要**由文字直接生成**剪影，用本 skill 之外、
由 `_paths.py` 的 `sd_cpp` 键定位的 `make-icon.ps1`（路径随机器而变，本机实测存在）。
