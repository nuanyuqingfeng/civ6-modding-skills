...
> 生成于 2026-09-14 的 12 工程横向审查；各 skill 的详细内容以各自 `SKILL.md` 为准。
...
| Skill | 版本 | 管什么（一句话） | 体量 |
|---|---|---|---|
| **`civ6-modding`** | 2.0 | **玩法侧总入口**：Lua（UI + GP）、ForgeUI XML、`.civ6proj`/`.modinfo`、数据库 XML/SQL、事件系统、API 参考；含 907 ModifierTypes / 1987 Effects / 545 Requirements 离线库与查询工具 | 218 MB（含 4 个 SQLite 快照） |
...
| `civ6-leader-2d` | 1.0 | **2D 领袖（立绘纸片人）**：XLP / Leaders.artdef / 几何 / 材质 / 灯光 / 环境光 / 贴图 / ast 全套注册链 | 0.09 MB |
| `civ6-loyalty-icon` | 1.0 | **忠诚度 / 宗教压力图标**：文明图标 + 光晕合成 + 注册链（含宗教分支） | 0.26 MB |
| `civ6-promotion-icon` | 1.0 | **单位晋升图标**：四类金属盾形徽章 + 白色剪影合成 | 3.07 MB |
| `civ6-governor-art` | 1.0 | **总督素材**：徽章 / 头像小图标 / 立绘 / 边缘透明渐变 | 0.12 MB |
| `civ6-tuner` | — | **FireTuner 运行时验证**（TCP 4318）：在运行中的对局里执行 Lua，回答"这个 API 实际行为是什么" | 0.09 MB |
| `civ6-workshop-uploader` | — | **Steam 工坊上传**：workspace 准备→构建→validate→upload→Steam API 验证 + 代理诊断 | 0.03 MB |
...
| **新增总督 / 改晋升树 / 就职回合 / 名额扩容** | `civ6-modding` → **`governor-authoring.md`**（美术另见 `civ6-governor-art`） |
...
| 做 2D 领袖立绘 / 忠诚度图标 / 宗教图标 / 晋升图标 / 总督素材 | 对应专项 skill（见 §一） |
| **运行时验证 API 行为 / 复现脚本报错** | `civ6-tuner` |
| 上传 / 更新 Steam 工坊 | `civ6-workshop-uploader` |
...
| Skill | 明确不做 |
|---|---|
| `civ6-modding` | 不承载美术引用链与 cook 层逻辑（→ `civ6-art-reference`）；不承载音频 bank 生成（→ `civ6-audio-pipeline`）；不承载工坊上传（→ `civ6-workshop-uploader`） |
| `civ6-art-reference` | **不解包任何 `.blp`**（一律按名引用）；2D UI 图标默认不处理（仅悬空时补链）；领袖立绘/文明图标走 `civ6-leader-2d` / `civ6-loyalty-icon` |
| `civ6-leader-2d` | 素材处理前**必须先询问用户是否提供素材**，默认只生成注册文件 |
| `civ6-loyalty-icon` | 2D UI 宗教图标（`IconTextureAtlases` 270px 图集）不在范围 |
| `civ6-governor-art` | 只管美术规格；总督的**玩法注册链**在 `civ6-modding/governor-authoring.md` |
...
| 其余 8 个 | ✅ | 2026-09-14 纳入版本控制，各含 `.gitignore` / `.gitattributes` |
